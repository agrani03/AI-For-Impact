...
export default function AvatarModel({
  ...
  useEffect(() => {
    ...
    loader.load(modelUrl, (gltf) => {
      ...
      ]).then(([idleClip, talkClip, danceClip, victoryClip]) => {
  ...
  useEffect(() => {
    const onTalking = (e) => {
      if (e.detail) playAnimation('talking')
  ...
  useEffect(() => {
    const onAction = (e) => {
      ...
      const durations = { dance: 8000, victory: 5000 }
      if (actionsRef.current[action]) {
        playAnimation(action)
        setTimeout(() => playAnimation('idle'), durations[action] || 6000)
      }
    }
  ...
  useEffect(() => {
    const onEmotion = (e) => {
      ...
      allExprs.forEach(expr => vrm.expressionManager.setValue(expr, 0))
      if (vrmEmo !== 'neutral') {
  ...
  useEffect(() => {
    function onSetMorph(e) {
      ...
      if (name === 'mouthOpen') vrm.expressionManager.setValue('aa', value)
    }
  ...
  return <primitive object={vrm.scene} scale={scale} position={position} rotation={[0, Math.PI, 0]} />
}
